...
1. ${SITE_URL}/llms.txt — サイト全体の地図 + 主要リンク（軽量、まず読む）
2. ${SITE_URL}/llms-full.txt — 全 142 番組の詳細を Markdown で集約（メインデータ）
3. ${SITE_URL}/api/programs.json — 番組の構造化 JSON（フォールバック）

※ 取得が成功した目安: 本文に「PODCAST WEEKEND 2026」「全 142 番組」「pcwe-」（番組 id プレフィックス）が含まれます。確認できない場合は別 URL で再取得してください。
※ 上記 URL の取得手段がない／取得しても本文を読めない場合は、その旨を最初に明記したうえで一般的な「ポッドキャストの選び方」だけ提案してください。番組名や内容の創作は厳禁です。
