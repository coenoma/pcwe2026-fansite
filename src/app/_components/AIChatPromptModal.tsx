...
### 推奨ルート A: 集約データ（一発で全 142 番組）

1. ${SITE_URL}/llms-full.txt — 全 142 番組の詳細（テキスト、約 260KB）
2. ${SITE_URL}/api/programs.json — 同じデータの JSON 版（約 320KB）
3. ${SITE_URL}/llms.txt — サイト地図（約 36KB、軽量）

### フォールバック・ルート B: HTML ページ巡回（A が読めない時）

A の URL は取得できるが本文が空 / メタ情報のみで返るケースがあります。その場合は次の HTML ページを順に取得してください（HTML はどの AI クライアントでも本文展開できます）:

1. ${SITE_URL}/ — トップページ（イベント概要 + 主要セクション + 全 142 番組への ItemList JSON-LD 構造化データを含む）
2. ${SITE_URL}/genre/{ジャンル名} — ジャンル別の番組一覧（カルチャー / 食 / 音楽 / 旅 など、トップから辿れる）
3. ${SITE_URL}/booth/pcwe-XXX — 個別番組詳細（XXX は 001〜142 の 3 桁、推薦候補が見えてきたら個別に取得して根拠を補強）

ルート B で進めた場合は、回答の最初に「※ HTML ページから取得した情報をもとに提案します」と一言添えてください（情報源の透明性のため）。
