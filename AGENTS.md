...
### 物販詳細 description の改行ルール（v1.10.1 追加）

`merchandiseDetails[].description` は UI 側で `whitespace-pre-line` 付きの `<p>` で
レンダリングされるため、**JSON 内の `\n`（単一改行）と `\n\n`（空行）が表示にそのまま反映** される。
長文の description は読みやすさのために以下のルールで段落分けする運用：

- 文末「と告知。」「と発表。」「と公開。」のような一区切り → `\n\n`（空行）で次の文と分離
- 「：」「:」付きの見出し（「物販ラインナップ：」「グッズハイライト：」「特別企画：」等）→ `\n` で改行 + 次行から内容
- 項目が 3 件以上ある場合 → `- 項目1\n- 項目2\n- 項目3` の箇条書き形式
- 末尾の補足情報（「無料配布あり」「現金のみ」等）→ 空行を挟んで別段落

例:

```jsonc
"description": "Instagram 投稿（@host）にて『…』と告知。\n\nグッズハイライト：\n- 商品 A（説明）\n- 商品 B\n- 商品 C\n\n無料配布あり。"
```

既存データの一括整形は `python3` で `re.sub` を使った段落分け / 箇条書き化のヒューリスティック
を適用済（v1.10.1 リリース時 17 ファイル）。新規データ追加時もこの形式を踏襲すること。
