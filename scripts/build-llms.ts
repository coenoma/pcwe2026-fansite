...
- [llms-full.html](${SITE_URL}/llms-full.html): **AI / LLM クライアント向けに最適化された HTML 版**（text/plain や JSON が web sandbox で本文展開されない場合の確実なフォールバック。同データを HTML 形式で 1 ページに集約）
...
<link rel="canonical" href="${SITE_URL}/llms-full.html">
...
function main(): void {
  ...
  // 2.5. llms-full.html（LLM 向け HTML 版 / ChatGPT 等の sandbox 制約対策）
  const llmsFullHtml = buildLlmsFullHtml(programs);
  writeFileSync(join(ROOT, 'public/llms-full.html'), llmsFullHtml, 'utf-8');
  console.log(`✅ public/llms-full.html 生成 (${llmsFullHtml.length} 文字)`);
