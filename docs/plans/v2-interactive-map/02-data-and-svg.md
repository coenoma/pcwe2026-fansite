...
| サブタスク | 状況 |
|---|:---:|
| `data/booth-positions.json` 確定（テント 1-29、175 スロット）| ✅ |
| 公式画像 webp 配置 | ✅ |
| 3 番組 external 参照（PodWalker / まかないラジオ / アイデア刺激法）| ✅ |
| テント 30, 31 placeholder | ✅ |
| SVG 座標起こし（polygon 追記）| ⏳ |
| `src/lib/types.ts` スキーマ拡張（`ExhibitionSchema.position` 等）| ⏳ |
| `scripts/validate-booth-positions.ts` | ⏳ |
| `scripts/auto-tag-merchandise.ts` | ⏳ |
...
### 5-2. 公式画像の DL 機能（ユーザー要件追加 / Q4 確定: UIUX は Claude 判断）

公式画像 `pcwe2026-day1.webp` / `pcwe2026-day2.webp` は **マップ画面から DL 可能** にする。

#### 配置: マップ下部の独立 "公式画像セクション"（Q4 への私の判断）

フローティング案・上部バー案・下部ユーティリティバー案を比較して、**マップの下に独立したセクション** として配置するのが最も使いやすい:

理由:
- マップを操作している途中に視界に入らない（中断されない）
- 「公式画像が欲しい」と意識して下にスクロールする時の発見性が高い
- スマホで指が届きやすい下部
- ライセンス透明性の文言を一緒に表示できる
...
実装:
- `<a href="/images/map/pcwe2026-day1.webp" download="pcwe2026-day1.webp">` で DL
- サムネは `next/image` で webp を読み込み（width/height 必須）
- スマホでは縦並び、PC では横並び 2 列

意図:
- 当日電波が弱い前提のフォールバック（公式画像をスマホに保存しておけば確実）
- コエノママップ側のバグ・誤情報があった場合の保険
- ユーザー指示の「動線を設けて表示や DL ができるように」を満たす

### 5-3. ライセンス整理（ユーザー Q6 確定: 事前報告しない）

公式 JPEG マップを **そのまま再配布する場合** はライセンス確認が必要。

本プロジェクトのスタンス（Q6 で確定）:
- **公式 webp 画像をそのまま `public/images/map/` に配置 + DL 提供**: 「リスナーの利便性のためのキャッシュ的提供」と整理
- **独自 SVG マップ**: 公式画像をもとに、独自にマップを描き直し（ロゴ・色・タイポは完全独自）
- **明記する文言（マップ下部に常設）**:

  ```
  公式画像をもとに、独自にマップを描き直してます。
  公式画像はこちらから: [⬇ Day1 公式画像 DL] [⬇ Day2 公式画像 DL] [🔗 PCWE 公式サイト]
  ```

- マップ下部に「位置情報の出典: PCWE2026 公式会場マップ／©PODCAST WEEKEND 2026」を明記
- 公式サイト [podcastexpo.jp](https://podcastexpo.jp/) へのリンク常設

**Q6 確定により事前報告はしない**: 公式画像を出典明記して再配布 + 独自 SVG マップ + 透明な動線で OK。
