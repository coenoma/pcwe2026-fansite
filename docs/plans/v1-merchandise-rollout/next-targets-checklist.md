# 次ターゲット チェックリスト（B群残り 14 件）

**作成日**: 2026-05-06 22:00
**対象**: not-found 残り 41 件のうち、Listen 登録ありで再探索余地あり = **14 件**

---

## 経緯

A1 群（Listen 未登録 + SNS あり）は全 56 件完走済み（37 done / 19 not-found）。
次は B 群残り 14 件＝Listen 登録あるが過去探索で PCWE 関連エピなしと判定した番組。
5/6 朝〜昼の探索後、新エピソードが出ている可能性あり（特に 5/9 直前の今日明日）。

---

## 並行作業ルール（A1 群と同じ）

- **👤 菊池さん（探索の主担当）**: ブラウザログイン状態で X / IG タイムラインを直接スクロール、
  または Listen の番組ページで最新エピのタイトルを確認 → URL をチャットに貼る
- **🤖 Claude（反映専任）**: 受け取った URL を syndication / Listen MCP / WebFetch で詳細取得 →
  `merchandiseDetails` 反映 → コミット / push / main マージ
- **発見 URL 報告**: 「pcwe-XXX 発見 URL」
- **not-found 報告**: 「pcwe-XXX 言及なし」

---

## 状態凡例

- ⏳ 未着手（5/6 朝〜昼に Claude 単独で確認 → 当時は PCWE 関連なし、新エピ次第で再判定）
- 🔍 探索中
- ✅ done 化済み
- ❌ 告知なし確定（5/6 夕方以降の最新エピ含めて再確認後）
- 👀 候補あり要判断

---

## チェックリスト

| 状態 | ID | 番組名 | X | Instagram | Listen | 公式merch | 発見 URL / メモ | 更新日 |
|---|---|---|---|---|---|---:|---|---|
| ✅ | pcwe-007 | 自分にやさしくするラジオ | https://x.com/sophieauthentic | — | https://listen.style/p/begentlewithyourself | 5 | https://x.com/SOPHIEauthentic/status/2051621345700770239 番組英語タイトル彫り入りノート | 05-06 |
| ✅ | pcwe-014 | オバトーク | https://x.com/obatalk2024 | — | https://listen.style/p/obatalk | 4 | https://x.com/OBATalk2024/status/2051671413581656066 ポーチ・モババ・タロットおみくじ・対戦ゲーム | 05-06 |
| 🔄 | pcwe-019 | 高橋クリスのFA_RADIO:工場自動化ポッドキャスト | https://x.com/fulhause | — | https://listen.style/p/faradio | 6 | 菊池さん目視で PCWE 言及なし確認 | 05-06 |
| ✅ | pcwe-033 | HYPLACE WAVE #はいなみ | https://x.com/hainami_758 | https://www.instagram.com/mash_january25 | https://listen.style/p/hainami758 | 3 | X 2件: PODRAPユニット🔊BUZZY NIGHT🌃 デビュー曲最速頒布 + 音楽CD・ステッカー | 05-06 |
| 🔄 | pcwe-041 | 酒の道～日本酒の聖地から～@しぶさわくんFM | https://x.com/shibusawakunfm | — | https://listen.style/p/w6pm3kfn | 2 | 菊池さん目視で PCWE 言及なし確認 | 05-06 |
| 🔄 | pcwe-046 | 人生百貨店 | https://x.com/lifedeptstores | https://www.instagram.com/lifedepartm... | https://listen.style/p/lifedeptstores | 3 | 菊池さん目視で PCWE 言及なし確認 | 05-06 |
| 🔄 | pcwe-047 | クリエイターエコノミーニュース | https://x.com/creator_enews | — | https://listen.style/p/creator_enews | 2 | 菊池さん目視で PCWE 言及なし確認、当日まで monitoring | 05-06 |
| ✅ | pcwe-061 | さのみきひとのラジオ ⌞ ラのみきジオ ⌝ | https://x.com/ranomikijio | — | https://listen.style/p/ranomikijio | 4 | https://x.com/ranomikijio/status/2047888758092296639 NEW GOODS 3点（円盤・キラ角シール・紙束）| 05-06 |
| 🔎 | pcwe-106 | 公共訴訟ラジオ｜社会を動かす裁判の話 | https://x.com/CALL4_Jp | https://www.instagram.com/call4_jp/ | https://listen.style/p/cem9oa2z | 5 | Claude が X/IG/Listen/公式サイト確認したが PCWE2026 言及なし。X/IG タイムラインへの構造的アクセス手段限界で not-found 維持 | 05-06 |
| 🔄 | pcwe-107 | ラジオただいま発酵中 | — | — | https://listen.style/p/cnjfbfzz | 4 | 菊池さん目視で PCWE 言及なし、当日まで monitoring | 05-06 |
| 🔄 | pcwe-109 | しゃらくさラジオ | https://x.com/SyarakusaRadio | — | https://listen.style/p/syarakusaradio | 4 | 菊池さん目視で PCWE 言及なし、当日まで monitoring | 05-06 |
| 🔄 | pcwe-115 | 吉村ジョナサンの高校古典講義 | https://x.com/dai_koku_sama | https://www.instagram.com/book_bridge... | https://listen.style/p/yoshimurajona | 3 | 菊池さん目視で PCWE 言及なし、当日まで monitoring | 05-06 |
| ✅ | pcwe-121 | ぜったい大丈夫だよラジオ | https://x.com/daijobu_radio | — | https://listen.style/p/6ki02xqz | 6 | https://x.com/daijobu_radio/status/2052002184255111246 5/9 ブース17-C 秘密のコミック・限定音源・NEW GOODS | 05-06 |
| 🔄 | pcwe-139 | ぬまずっきゅーんfromみんキャス | https://x.com/numazukyun | https://www.instagram.com/numazukyun/ | https://listen.style/p/edjfkyvh | 2 | 菊池さん目視で PCWE 言及なし、当日まで monitoring | 05-06 |

---

## サマリ

- ⏳ 未着手: 0 件 🎉 **B群 14 件完走**
- 🔍 探索中: 0 件
- ✅ done 化済み: 5 件（pcwe-007, 014, 033, 061, 121）
- 🔄 monitoring（菊池さん目視で言及なし、当日まで再チェック余地あり）: 8 件（pcwe-019, 041, 046, 047, 107, 109, 115, 139）
- 🔎 not-found（Claude 探索手段限界）: 1 件（pcwe-106）
- 👀 候補あり要判断: 0 件

---

## 関連ドキュメント

- [README.md](./README.md) — 全体方針
- [a1-sns-checklist.md](./a1-sns-checklist.md) — A1 群（完走済み）
- [listen-rechecklist.md](./listen-rechecklist.md) — B 群初回探索の判定結果
- [monitoring.md](./monitoring.md) — 言及なし・要再チェック（菊池さん再確認候補）